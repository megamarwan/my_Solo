...
export default function UploadForm() {
 const [title, setTitle] = useState("");
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    // Define your consistent path
    const targetFolder = 'Documents/merchandise';

    try {
      // 1. Create the directory inside Documents. 
      // recursive: true ensures the full path is built if missing.
      await puter.fs.mkdir(targetFolder, { recursive: true }).catch(() => null);

      // 2. Write the file to the new location
      await puter.fs.write(`${targetFolder}/item_${id}.json`, JSON.stringify(newObject));
      
      console.log(`Successfully saved to ${targetFolder} in Puter!`);
      navigate('/profile');
    } catch (err: any) {
